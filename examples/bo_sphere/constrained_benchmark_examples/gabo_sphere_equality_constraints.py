...
function. In this example, the search domain is constrained onto a great circle of the sphere and represents a subspace 
of the manifold.
...
the BO observation with dots. The optimization domain is displayed as a black circle.
...
if __name__ == "__main__":
    ...
    # Define the solver (options: CTR or ALM)
    solver_name = 'ALM'

    # Instantiate the manifold
    sphere_manifold = pyman_man.Sphere(dim)

    # Define constraints (here: equality-constraints y=0)
    ...
    def y_great_circle(x):
        return x[1] - yc

    eq_constraints = [y_great_circle]
    ...
    def sample_sphere_constrained():
        ...
        d = 1
        x[d] = yc
        x[indices != d] = x[indices != d] / np.linalg.norm(x[indices != d]) * np.sqrt((1 - x[d] ** 2))
        return x
    ...
    if disp_fig:
        ...
        r_circle = np.sqrt(1. - yc ** 2)
        yc_circle = np.vstack((r_circle * circle[0], yc * np.ones(nbDrawingSeg), r_circle * circle[1]))
        ax.plot(xs=yc_circle[0, :], ys=yc_circle[1, :], zs=yc_circle[2, :], color='k')
    ...
    for iteration in range(n_iters):
        ...
        new_x = joint_optimize_manifold(acq_fct, sphere_manifold, solver, q=1, num_restarts=5, raw_samples=100,
                                        bounds=bounds, equality_constraints=eq_constraints)
    ...
    if disp_fig:
        ...
        ax1.plot([0., 1.], [yc, yc], zs=[true_opt_val[0, 0], true_opt_val[0, 0]], color='k')
        ax2.plot([yc, yc], [0., 1.], zs=[true_opt_val[0, 0], true_opt_val[0, 0]], color='k')
