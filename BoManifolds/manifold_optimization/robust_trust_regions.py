...
class TrustRegions(Solver):
    """
    Instances of this class are solvers using the trust-regions algorithm for optimization on manifolds.
    ...
    def _truncated_conjugate_gradient(self, problem, x, fgradx, eta, Delta,
        ...
        for j in xrange(0, int(maxinner)):
            ...
            # that d_Hd is not zero to avoid raising exceptions (ADDED PART compared to pymanopt implementation)
            if d_Hd != 0:
                alpha = z_r / d_Hd
                # <neweta,neweta>_P =
